"""WP Form Debugger page: enterprise WordPress form diagnostics.

Rendered inside the hub app. All analysis logic lives in core.py, which has no
Streamlit dependency and can be reused behind a CLI or an API.
"""
...
def render() -> None:
    ...
    with tab_audit:
        ...
        if html and html.strip():
            ...
            st.markdown(
                f"""
    <div class="app-kpis">
      <div class="app-kpi crit"><div class="n">{counts['critical']}</div><div class="l">Critical</div></div>
      <div class="app-kpi warn"><div class="n">{counts['warning']}</div><div class="l">Warnings</div></div>
      <div class="app-kpi"><div class="n">{counts['info']}</div><div class="l">Informational</div></div>
      <div class="app-kpi ok"><div class="n">{len(findings)}</div><div class="l">Checks flagged</div></div>
    </div>
    """,
                unsafe_allow_html=True,
            )
    ...
    <div class="app-card {cls}">
      <h4><span class="app-tag {cls}">{SEVERITY_LABEL[f.severity]}</span>{esc(f.title)}</h4>
      <div class="app-ev">{esc(f.evidence)}</div>
    ...
    with tab_smtp:
        ...
        if result:
            ...
            rows = "".join(
                f'<div class="app-stage"><div class="s {esc(s)}">{esc(s.upper())}</div>'
